...
const resolveNeededSecrets = (plugins) => {
  ...
  secrets.forEach((secretName, key) => {
    // FIXME: Add tests for the "?" suffix
    if (secretName.endsWith('?')) {
      const suffixLessSecretName = secretName.slice(0, -1);
      secrets[key] = secrets.includes(suffixLessSecretName) ? undefined : suffixLessSecretName;
    } else {
  ...
  return secrets.reduce((result, secretName) => {
    /** @type {EnvSyncSecret} */
    result[secretName] = {
      required: requiredSecrets.includes(secretName),
